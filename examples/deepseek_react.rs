//! Runs a complete `DeepSeek` -> Rust tool -> `DeepSeek` `ReAct` loop.

use std::error::Error;

use agentscope::{
    Msg, OpenAIChatModel, ReActAgent, Tool, ToolContext, ToolDefinition, ToolError, ToolExecutor,
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let reply = agent
        .reply(Msg::user("What is 123 multiplied by 456?"))
        .await?;
